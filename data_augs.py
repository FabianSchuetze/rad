...
def random_rotation(images, p=.3):
    ...
    for i, m in enumerate(masks):
        ...
        m = m[:, None] * torch.ones([1, frames]
                                    ).type(mask.dtype).type(images.dtype).to(device)
        m = m[:, :, None, None]
        masks[i] = m
...
if __name__ == '__main__':
    ...
    print(
        tabulate(
            ...
            headers=['Data Aug', 'Time / batch (secs)',
                     'Time / 100k steps (mins)']))
